...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('katware/assets/new/blur.png')
...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		katwareEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
...
run(function()
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				katwareEvents.InventoryChanged:Fire()
			end
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'EntityDamageEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not katware.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			katware:Clean(connection:Connect(function(...)
				katwareEvents[event]:Fire(...)
			end))
		end)
	end
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		bedwars.ClientDamageBlock:WaitFor(event):andThen(function(connection)
			if not katware.Connections then return end
			katware:Clean(connection:Connect(function(data)
				for i, v in cache do
...
run(function()
	...
	katware.Categories.Combat:CreateModule({
		Name = 'NoClickDelay',
		Function = function(callback)
			if callback then
...
local function CheckForNearbyPlayers(player, radius)
...
run(function()
	...
	GodMode = katware.Categories.Blatant:CreateModule({
		Name = "Godmode",
		Function = function(callback)
			if callback then
				task.spawn(function()
					repeat task.wait()
						pcall(function()
							if (not katware.Modules.Fly.Enabled) and (not katware.Modules.InfiniteFly.Enabled) then
								for _, v in pairs(entitylib.List) do
									if v.Player and v.Player.Team ~= lplr.Team and IsAlive(v.Player) and IsAlive(lplr) then
										if v and v.Player ~= lplr then
					...
					until (not GodMode.Enabled == true)
				end)
			end
		end
	})
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if katware.Modules.Fly.Enabled or katware.Modules.InfiniteFly.Enabled or katware.Modules.LongJump.Enabled then
	...
	Fly = katware.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(katwareEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	InfiniteFly = katware.Categories.Blatant:CreateModule({
		Name = 'InfiniteFly',
		Function = function(callback)
			frictionTable.InfiniteFly = callback or nil
			updateVelocity()
			if callback then
				if katware.Modules.Invisible and katware.Modules.Invisible.Enabled then
					katware.Modules.Invisible:Toggle()
...
run(function()
	...
	local anims, AnimDelay, AnimTween, armC0 = katware.Libraries.auraanims, tick()
...
run(function()
	...
	LongJump = katware.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			updateVelocity()
			if callback then
				LongJump:Clean(katwareEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(katwareEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	BedESP = katware.Categories.Render:CreateModule({
		Name = 'BedESP',
		Function = function(callback)
			if callback then
	...
	Health = katware.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Health:Clean(label)
				Health:Clean(katwareEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	KitESP = katware.Categories.Render:CreateModule({
		Name = 'KitESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	StorageESP = katware.Categories.Render:CreateModule({
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = katware.Categories.Utility:CreateModule({
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoKit = katware.Categories.Utility:CreateModule({
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPlay = katware.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoPlay:Clean(katwareEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(katwareEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	AutoToxic = katware.Categories.Utility:CreateModule({
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoVoidDrop = katware.Categories.Utility:CreateModule({
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = katware.Categories.Utility:CreateModule({
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	RavenTP = katware.Categories.Utility:CreateModule({
		Name = 'RavenTP',
		Function = function(callback)
			if callback then
				RavenTP:Toggle()
...
run(function()
	...
	local function staffFunction(plr, checktype)
		if not katware.Loaded then repeat task.wait() until katware.Loaded end
		notif('katware: ', 'Staff Detected ('..checktype..'): '..plr.Name..' ('..plr.UserId..')', 60, 'alert')
	...
	local function playerAdded(plr)
		...
		if getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and katware.Loaded then
				connection:Disconnect()
	...
	task.spawn(function()
		repeat task.wait(1) until katware.Loaded or katware.Loaded == nil
		if katware.Loaded and not StaffDetector.Enabled then
...
run(function()
	StoreDamage = katware.Categories.Utility:CreateModule({
		Name = 'StoreDamage',
...
run(function()
	TrapDisabler = katware.Categories.Utility:CreateModule({
		Name = 'TrapDisabler',
...
run(function()
	...
	AutoTool = katware.Categories.World:CreateModule({
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedProtector = katware.Categories.World:CreateModule({
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = katware.Categories.World:CreateModule({
		Name = 'Schematica',
		Function = function(callback)
			if callback then
	...
	AutoConsume = katware.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				AutoConsume:Clean(katwareEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		window.Parent = katware.gui.ScaledGui
		...
		close.MouseButton1Click:Connect(function()
			window.Visible = false
			katware.gui.ScaledGui.ClickGui.Visible = true
		end)
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / katware.guiscale.Scale)
		end)
		table.insert(katware.Windows, window)
	...
	katware.Components.HotbarList = function(optionsettings, children, api)
		if katware.ThreadFix then
			setthreadidentity(8)
		end
		local optionapi = {
		...
		textbuttonicon.Image = getcustomasset('katware/assets/new/add.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / katware.guiscale.Scale, 603))
		end)
		...
		function optionapi:AddHotbar(data)
			...
			hotbar.MouseButton1Click:Connect(function()
				...
				if ind == optionapi.Selected then
					katware.gui.ScaledGui.ClickGui.Visible = false
	...
	AutoHotbar = katware.Categories.Inventory:CreateModule({
		Name = 'AutoHotbar',
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(katwareEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = katware.Categories.Inventory:CreateModule({
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = katware.Categories.Inventory:CreateModule({
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedPlates = katware.Categories.Minigames:CreateModule({
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
				for _, v in collectionService:GetTagged('bed') do 
				...
				BedPlates:Clean(katwareEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(katwareEvents.BreakBlockEvent.Event:Connect(refreshNear))
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('katware/assets/new/blur.png'),
...
run(function()
	...
	local Crosshair = katware.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	DamageIndicator = katware.Legit:CreateModule({
...
run(function()
	...
	FPSBoost = katware.Legit:CreateModule({
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = katware.Legit:CreateModule({
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = katware.Legit:CreateModule({
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = katware.Legit:CreateModule({
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = katware.Legit:CreateModule({
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SoundChanger = katware.Legit:CreateModule({
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	UICleanup = katware.Legit:CreateModule({
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do 
...
run(function()
    ...
    function handlers:enable()
		antiDeath:Clean(runService.Heartbeat:Connect(function()
            ...
            if entitylib.character.Humanoid.Health <= antiDeathConfig.Health.Value and entitylib.character.Humanoid.Health > 0 then
    ...
    function handlers:enableInfiniteMode()
        if not katware.Modules.InfiniteFly.Enabled then
            katware.Modules.InfiniteFly:Toggle(true)
            self.inf = true
        end
    end
    ...
    function handlers:applyBoost()
        local boostActions = {
            ...
            Tween = function()
                tweenService:Create(self.hrp, TweenInfo.new(antiDeathConfig.TweenDuration.Value / 10), {
	...
	local function InfoNotification(...)
		katware:CreateNotification(...)
	end
    function handlers:sendNotification()
        InfoNotification('AntiDeath', 'Prevented death. Health is lower than ' .. antiDeathConfig.Health.Value ..
            '. (Current health: ' .. math.floor(entitylib.character.Humanoid.Health + 0.5) .. ')', 5)
        self.hasNotified = true
    end

	local function playSound(id)
		if bedwars and bedwars.SoundManager then
    ...
    function handlers:resetMode()
        if self.inf then
            if antiDeathConfig.AutoDisable.Enabled then
                if katware.Modules.InfiniteFly.Enabled then
                    katware.Modules.InfiniteFly:Toggle(false)
                end
            end
            self.inf = false
            self.hasNotified = false
	...
	local function getHealth()
		return entitylib.character and entitylib.character.Humanoid and entitylib.character.Humanoid.Health or 0
	end

    antiDeath = katware.Categories.Utility:CreateModule({
        Name = 'AntiDeath',
        Function = function(callback)
            if callback then
    ...
    antiDeathConfig.BoostMode = antiDeath:CreateDropdown({
        ...
        Default = 'Velocity',
        Tooltip = 'Mode to boost your character.',
        Function = function(val)
    ...
    antiDeathConfig.Health = antiDeath:CreateSlider({
        ...
        Min = 10,
        Max = 90,
        Tooltip = 'Health at which AntiDeath will perform its actions.',
        Default = 50,
    ...
    antiDeathConfig.Velocity = antiDeath:CreateSlider({
        ...
        Min = 100,
        Max = 600,
        Tooltip = 'Power to get boosted in the air.',
        Default = 600,
        Function = function(val) end
    })
    antiDeathConfig.Velocity.Object.Visible = false
    ...
    antiDeathConfig.TweenDuration = antiDeath:CreateSlider({
        ...
        Min = 1,
        Max = 10,
        Tooltip = 'Duration of the tweening process.',
        Default = 4,
        Function = function(val) end
    })
    antiDeathConfig.TweenDuration.Object.Visible = false
    ...
    antiDeathConfig.SkyPosition = antiDeath:CreateSlider({
        ...
        Min = 100,
        Max = 1000,
        Tooltip = 'Position to TP in the sky.',
        Default = 1000,
        Function = function(val) end
    })
    ...
    antiDeathConfig.AutoDisable = antiDeath:CreateToggle({
        Name = 'Auto Disable',
        Tooltip = 'Automatically disables InfiniteFly after healing.',
        Function = function(val) end,
        Default = true
    })
    ...
    antiDeathConfig.Sound = antiDeath:CreateToggle({
        Name = 'Sound',
        Tooltip = 'Plays a sound after preventing death.',
        Function = function(callback)
    ...
    antiDeathConfig.Notify = antiDeath:CreateToggle({
        Name = 'Notification',
        Tooltip = 'Notifies you when AntiDeath actioned.',
        Default = true,
...
run(function()
    ...
    local adetunde_remotes = {
        ["Shield"] = function()
            local args = { [1] = "shield" }
            local returning = replicatedStorage
    ...
    AdetundeExploit = katware.Categories.Blatant:CreateModule({
                                    ...
                                    if Strength == 3 or Speed == 3 or Shield == 3 then
                                        if (Strength == 3 and Speed == 2 and Shield == 2) or
                                           ...
                                           (Strength == 2 and Speed == 2 and Shield == 3) then
                                            notif("Katware AdetundeExploit", "Fully upgraded everything possible!", 7)
...
run(function()
    local Autowin = { Enabled = false }
    local AutowinNotification = { Enabled = true }
    local bedtween, playertween
    local lastActionTime = 0
    local failedTweenAttempts = 0
    
    -- Configuration
    local actionDelay = 1       -- Delay after respawn
    local killDelay = 2.5      -- Delay between kills
    local tweenTimeout = 3     -- Tween timeout duration
    local targetSearchRange = 30  -- Initial search range
    local maxSearchRange = 75    -- Maximum search range
    local searchIncrement = 20   -- Search range increment
    local maxFailedAttempts = 3  -- Maximum failed attempts before reset
    local maxTweenDistance = 10  -- Maximum acceptable distance after tween
    ...
    local function validatePosition(current, target, maxDist)
        ...
        return dist <= (maxDist or maxTweenDistance)
    end
    ...
    local function handleTargetSearch()
        local target = FindTarget(targetSearchRange, true)
        if target and target.RootPart and IsAlive(lplr) then
            targetSearchRange = 30  -- Reset search range on successful find
            failedTweenAttempts = 0
            return target
        else
            if targetSearchRange < maxSearchRange then
    ...
    local function tweenToTarget(target)
        ...
        playertween.Completed:Wait()
        task.wait(0.2)  -- Short delay after tween
        
        -- Validate final position
        if not validatePosition(lplr.Character.HumanoidRootPart, target.RootPart) then
            notif("Autowin", "Tween failed - position mismatch", 3)
            resetCharacter()
            return false
        end
    ...
    local function handleBedDestruction(bed)
        ...
        task.delay(tweenTimeout, function()
            if bedtween and bedtween.PlaybackState == Enum.PlaybackState.Playing then
                notif("Autowin", "Bed tween timeout - resetting", 3)
                bedtween:Cancel()
                resetCharacter()
                return false
            end
        ...
        bedtween.Completed:Wait()
        task.wait(0.2)  -- Post-tween delay
        
        if not validatePosition(lplr.Character.HumanoidRootPart, bed) then
            notif("Autowin", "Failed to reach bed", 3)
            resetCharacter()
            return false
        end
        
        -- Wait for bed to be broken
        repeat task.wait() until FindEnemyBed() ~= bed or not IsAlive(lplr)
        return true
    end
	Autowin = katware.Categories.Blatant:CreateModule({
        ...
        Function = function(callback)
            if callback then
                task.spawn(function()
                    ...
                    if not katware.Loaded then
                        repeat task.wait() until katware.Loaded or not Autowin.Enabled
                    end
                    if not Autowin.Enabled then return end
                    ...
                    -- Setup heartbeat connection for desync checks
                    Autowin:Clean(runService.Heartbeat:Connect(function()
                        pcall(function()
                            local enemyBed = FindEnemyBed()
                            if not isnetworkowner(lplr.Character:WaitForChild("HumanoidRootPart")) and 
                               (enemyBed and GetMagnitudeOf2Objects(lplr.Character:WaitForChild("HumanoidRootPart"), enemyBed) > 75 or not enemyBed) then
                                if IsAlive(lplr) and FindTeamBed() and Autowin.Enabled and (not store.matchState == 2) then
                                    resetCharacter()
                                end
                            end
                    ...
                    -- Main character respawn handler
                    Autowin:Clean(lplr.CharacterAdded:Connect(function()
                        if not IsAlive(lplr) then repeat task.wait() until IsAlive(lplr) end
                        task.wait(actionDelay)  -- Wait after respawn

                        -- Check for enemy bed
                        local bed = FindEnemyBed()
                        if bed and (bed:GetAttribute("BedShieldEndTime") and bed:GetAttribute("BedShieldEndTime") < workspace:GetServerTimeNow() or not bed:GetAttribute("BedShieldEndTime")) then
                            -- Handle bed destruction
                            if not handleBedDestruction(bed) then return end
                            
                            -- Reset after bed destruction
                            resetCharacter()
                            task.wait(actionDelay)
                            lastActionTime = tick()

                            -- Target elimination loop
                            while Autowin.Enabled and IsAlive(lplr) do
                                if (tick() - lastActionTime) >= killDelay then
                                    -- Check for desync
                                    if handleDesync() then continue end
                                    ...
                                    if target then
                                        ...
                                        if not tweenToTarget(target) then
                                            ...
                                            if failedTweenAttempts >= maxFailedAttempts then
                                                notif("Autowin", "Too many failed attempts - resetting", 3)
                                                resetCharacter()
                                                failedTweenAttempts = 0
                                                continue
                                            end
                                ...
                                task.wait(0.1)  -- Prevent loop from running too fast
                            end

                        elseif FindTarget(nil, true) and FindTarget(nil, true).RootPart then
                            -- Handle immediate target if no bed
                            local target = FindTarget(nil, true)
                            if tweenToTarget(target) then
                                lastActionTime = tick()
                                task.wait(killDelay)  -- Wait for kill
        ...
        Tooltip = "Advanced autowin with improved targeting and movement"
    })
...
run(function()
    ...
    BedTP = katware.Categories.Utility:CreateModule({
        ...
        Function = function(callback)
            if callback then
                if not IsAlive(lplr) then 
                    notif("BedTP", "You must be alive to use BedTP!", 3)
                    BedTP:Toggle(false)
                    return
                end
...
run(function()
	...
	AutoPearl = katware.Categories.Utility:CreateModule({
		Name = 'AutoPearl',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	FastBreak = katware.Categories.Blatant:CreateModule({
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	ArmorSwitch = katware.Categories.Inventory:CreateModule({
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
