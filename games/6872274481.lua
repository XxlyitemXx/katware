...
run(function()
    ...
    local function executeSafeBedTween(bed)
        local success, bedtween = pcall(function()
            ...
            local completedConn, timeoutConn = safeTween(tween, 8)
            tween:Play()
    ...
    local function attackSequence(target)
        ...
        while attempts < maxAttempts and Autowin.Enabled and IsAlive(lplr) do
            ...
            pcall(function()
                ...
                local completedConn, timeoutConn = safeTween(playertween)
                playertween:Play()
                tweenSuccess = true
            end)
        ...
        Function = function(callback)
            if callback then
                task.spawn(function()
                    ...
                    repeat task.wait() until store.matchState ~= 0 and katware.Loaded
                    repeat task.wait() until lplr.Team and lplr.Team.Name ~= "Spectators"

                    local bed = FindEnemyBed()
                    if not bed then
                        notif("Autowin", "No enemy bed found!", 3)
                        return Autowin:Toggle(false)
                    end

                    -- Main logic loop
                    while Autowin.Enabled do
                        if tick() - lastFailureTime < COOLDOWN_DURATION then
                            task.wait(1)
                            continue
                        end

                        -- Phase 1: Bed destruction
                        local bedSuccess = pcall(function()
                            if not executeSafeBedTween(bed) then
                                error("Bed tween failed")
                            end
                        ...
                        if not bedSuccess then
                            if handleCriticalFailure("Bed phase failed") then
                                continue
                            end
                        end
                        ...
                        while Autowin.Enabled and IsAlive(lplr) do
                            ...
                            if target and target.RootPart then
                                local attackResult = attackSequence(target)
                                
                                if not attackResult then
                                    if handleCriticalFailure("Attack sequence failed") then
                                        break
                                    end
                                end
                            else
                        ...
                        -- Phase 3: Cleanup
                        if IsAlive(lplr) and FindTeamBed() then
