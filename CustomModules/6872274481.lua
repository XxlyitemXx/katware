					...
					if deathTable.finalKill then
						...
						if killed == lplr then 
							if (not leavesaid) and killer ~= lplr and AutoToxicDeath.Enabled then
								leavesaid = true
								local custommsg = #AutoToxicPhrases3.ObjectList > 0 and AutoToxicPhrases3.ObjectList[math.random(1, #AutoToxicPhrases3.ObjectList)] or 'I was too laggy <name>. That\'s why you won. and btw dont use this --> | .gg/voidware'
								if custommsg then
							...
							if killer == lplr and AutoToxicFinalKill.Enabled then 
								local custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or '<name> things could have ended for you so differently, if you NOT used Voidware. | kittykat'
								if custommsg == lastsaid then
									custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or '<name> things could have ended for you so differently, if you NOT used Voidware. | kittykat'
								else
									lastsaid = custommsg
								end
								if custommsg then
				...
				table.insert(AutoToxic.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					...
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						...
						if AutoToxicWin.Enabled then
							sendmessage(#AutoToxicPhrases.ObjectList > 0 and AutoToxicPhrases.ObjectList[math.random(1, #AutoToxicPhrases.ObjectList)] or 'Voidware is simply SUCK everyone. | katware')
						end
					end
				end))
					...
					if AutoToxicRespond.Enabled then
						...
						if plr and plr ~= lplr and not alreadyreported[plr] then
							...
							if reportreason then
								...
								local msg = custommsg or "oops.."..(plr.DisplayName or plr.Name).." | vxpe on top :D"
								if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
