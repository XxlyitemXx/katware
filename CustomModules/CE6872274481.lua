				...
				table.insert(AutoToxic.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					...
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						...
						if AutoToxicWin.Enabled then
							sendmessage(#AutoToxicPhrases.ObjectList > 0 and AutoToxicPhrases.ObjectList[math.random(1, #AutoToxicPhrases.ObjectList)] or 'Voidware is SUCK everyone. | Katware!')
						end
					end
				end))
					...
					if AutoToxicRespond.Enabled then
						...
						if plr and plr ~= lplr and not alreadyreported[plr] then
							...
							if reportreason then
								...
								local msg = custommsg or "I don't care about the fact that I'm hacking, I care about you dying in a block game. L "..(plr.DisplayName or plr.Name).." | vxpe on to frfr"
								if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
