...
local function InfoNotification(title, text, delay)
	local suc, res = pcall(function()
		local frame = GuiLibrary.CreateNotification(title or "katware", text or "Successfully called function", delay or 7, "assets/InfoNotification.png")
		return frame
	end)
    warn(title..": "..text)
	return (suc and res)
...
textlabel.Text = "Cracked VOIDWARE PRIVATE by @rinnnaaaa_"
...
task.spawn(function()
	pcall(function()
		repeat task.wait() until shared.GuiLibrary.ObjectsThatCanBeSaved["[katwere] ProfilesSaverOptionsButton"]
		local a = shared.GuiLibrary.ObjectsThatCanBeSaved["[katware] ProfilesSaverOptionsButton"]
